_notes

take 2 actions

---

CLOSE

NEARBY

FAR AWAY

0 - 5ft

5 - 60ft

60 - 120ft

---

PERCEPTION
When your character is actively looking for something, they find it. If time is of the essence, you should make a WIL check.

GROUP TEST
If half the characters succeed at their attribute check in a group effort, the whole group does.

During their turn, characters can take two actions.

Attacking 

Defending

Moving

TURNS

To make sure everyone at the table has a chance to speak, the game uses Turns. At the table, a Turn is needed for a character to move and act. A Turn may represent a few moments (during combat), a few minutes, a few hours, or even a few days. The game is always played in Turns.

ONE TURN, TWO ACTIONS

A character can make up to two actions during their turn to ensure no player hogs the spotlight. The action complexity (and the distance covered with a move action) depends on the length of the Turn established by the GM (moments, minutes, hours...). There's a catch, though, if you take the same action twice, you must make a Doom roll (see below).

DOOM

Violent and chaotic actions that bring the world closer to its end trigger a Doom roll. Depleting their Doom die means the characters bring doom to themselves. Roll Doom when:

- Taking the same action twice during a combat Turn*.
- Using a Gift that requires it.
- Getting a Critical failure on an attribute test.

The Doom die returns to its maximum (a d6) after the character takes a long rest.

*If the action already requires a Doom roll, roll with Disadvantage.

Players can choose to "invite" doom: roll Doom and subtract the result from one of your character's attribute checks or add it to a damage roll. The Doom die automatically degrades.

Once the Doom die is depleted, the character is considered Doomed. All their attribute tests and damage rolls are made with Disadvantage until the character can take a long rest.

| Barbarian | Civilized | Decadent |
| --------- | --------- | -------- |
| Berserker | Bookworm | Assassin |
| Herbalist | Inventor | Forbidden knowledge |
| Hunter | Legionnaire | Pit-fighter |
| Shaman | Sophist | Snale blood |
| Storyteller | Street Urchin | Vicious |
| Wildling | Swordmaster | Warlock |
[Backgrounds]

---

Example Gambits

Break a weapon

Disarm an opponent

Entangle

Pin down

Push 
